...
def plot_pareto(results_trace: dict, pareto_frontier: dict, plot_file: str):
    ...
    # Sort the Pareto points by y-value (descending)
    sorted_indices = sorted(range(len(pareto_bids["y"])), key=lambda i: pareto_bids["y"][i], reverse=True)
    sorted_x = [pareto_bids["x"][i] for i in sorted_indices]
    sorted_y = [pareto_bids["y"][i] for i in sorted_indices]

    # Add the sorted Pareto points
    fig.add_trace(
        go.Scatter(
            x=sorted_x,
            y=sorted_y,
    ...
    # Connect each point to the point with the next lowest y-value
    for i in range(len(sorted_y) - 1):
        fig.add_trace(
            go.Scatter(
                x=[sorted_x[i], sorted_x[i + 1]],
                y=[sorted_y[i], sorted_y[i + 1]],
                mode='lines',
